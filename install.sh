...
replace_once(
    ...
    "var ns=[`powershell`,`commandPrompt`],rs=[`--login`,`-i`];function is(){if(process.platform!==`win32`)return[];let e=[...ns];return ls()!=null&&e.push(`gitBash`),us()!=null&&e.push(`wsl`),e}function __codexValidShellPath(e){try{return typeof e==`string`&&e.length>0&&(0,a.existsSync)(e)&&((0,a.statSync)(e).mode&73)!==0}catch{return!1}}function __codexNormalizeTerminalCommand(e){if(process.platform===`win32`)return e;let t=Array.isArray(e)?[...e]:[],n=typeof t[0]==`string`?t[0].trim():``;for(let r of[n,process.env.SHELL?.trim(),`/bin/zsh`,`/bin/bash`,`/bin/sh`])if(__codexValidShellPath(r)){let i=r===n&&t.length>0?[...t]:[r],a=((i[0]??``).split(/[/\\\\]/).at(-1)||``).toLowerCase(),o=i.slice(1).filter(e=>e!==`-i`&&e!==`-l`&&e!==`--login`);if(a===`zsh`)return[i[0],`-f`,...o.filter(e=>e!==`-f`)];if(a===`bash`)return[i[0],`--noprofile`,`--norc`,...o.filter(e=>e!==`--noprofile`&&e!==`--norc`)];return[i[0],...o]}return[`/bin/sh`]}function as(t){if(process.platform!==`win32`){for(let t of[e.Pt(),process.env.SHELL?.trim(),`/bin/zsh`,`/bin/bash`,`/bin/sh`])if(__codexValidShellPath(t))return[t];return[`/bin/sh`]}",
